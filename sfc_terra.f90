...
!  Initial release for COSMO (TERRA-URB not yet activated)
...
! V5_7a        2020-05-11 Ulrich Schaettler, Astrid Kerkweg
!  Use the correct indices of the special debug point (US)
!  Only write plevap to debug output for itype_trvg=3 (US)
!  Update of MESSy interface implementation (AK)
! V5_7c        2020-07-03 Ulrich Schaettler
!  Update with ICON scheme to avoid NEC Compiler problems
!   esp: use snow-weighted potential evaporation to ensure that sum of 
!        evaporation terms does not exceed potential evaporation (Sect. I.4.4)
! V5_8         2020-10-23 CLM, Ulrich Schaettler, Daniel Regenass, Linda Schlemmer
!  Added tuning factors soilhyd, fac_rootdp2
!  Introduced new hydrology scheme for itype_hydmod=1
! V5_9a        2021-05-18 Astrid Kerkweg, Ulrich Schaettler
!  Made fr_wi publicly available, as it is required outside of TERRA for MESSy (AK)
!  Replaced lmulti_snow by itype_snow (US)
!  Remove variable flux_ksop05_neg from line 4229: DO-loop for "lowest active 
!    hydrological layer ke_soil_hy-1", where it is not really used (US)
! V5_9b        2021-06-02 Ronny Petrik
!  Fix for itype_hydmod=0: improve calculation of subsurface runoff to better close
!    the soil water budget
!  Compute the diagnostic field res_soilwatb
! V5_10        2021-07-21 Ulrich Schaettler, Hendrik Wouters, Mikhail Varentsov
!  Activate code for urban parameterization (HW,MV,US)
! V5_12        2021-09-09 Ulrich Blahak
!  Fixed k-index for diagnostic output
!
! Code Description:
! Language: Fortran 90.
...
! --------------------------
    iid_dbg, ibl_b, isc_b, &
    itype_snow,   & ! to choose the snow scheme
    ...
    itype_hydmod,  &! type of soil hydrology (0=standard, 1=Schlemmer et al 2018)
...
! --------------------------
    lres_soilwatb,& ! compute the soil water budget, residuum,
...
!
USE mo_lnd_nwp_config,     ONLY: itype_snow, l2lay_rho_snow,          &
...
! for TERRA-URB
                  isa              , & ! total impervious surface-area index           ( -- )
                                       ! (both urban and non-urban)
                  sa_uc            , & ! surface-area fraction of the urban canopy     ( --  )
                                       ! (buildings and pavements)
                  ai_uc            , & ! surface area index of the urban canopy        ( --  )
                                       ! (mostly larger than 1   )
                  alb_red_uc       , & ! albedo reduction factor for the urban canopy  ( --  )
                  urb_fr_bld       , & ! Building area fraction for TERRA-URB          (  -  )
                                       !   with respect to urban tile
                  urb_h2w          , & ! Street canyon H/W ratio for TERRA-URB         ( m/m )
                  urb_h_bld        , & ! Building height for TERRA-URB                 (  m  )
                  urb_alb_th       , & ! Urban material thermal albedo                 (  %  )
                  urb_alb_so       , & ! Urban material solar albedo                   (  %  )
                  urb_hcap         , & ! Volumetric heat capacity of urban material    (Jm-3K-1)
                  urb_hcon         , & ! Heat conductivity of urban material           (Wm-1K-1)
!
...
!
                  ...
                  res_soilwatb     , & ! soil water budget, residuum                   (kg/m2)
! for TERRA-URB
                  w_imp            , & ! impervious water storage                        --
                  w_isa            , & ! same, multiplied by isa                         --
!
                  ...
                  s_oro_max        , & ! gradient of subgridscale orography            (  1  )
                  wt_depth         , & ! water table depth                             (  m  )
                  wso_satlev       , & ! uppermost saturated level of water table      (  -  )
                  wso_sat          , & ! soil moisture saturation                      (  -  )
                  q_roff           , & ! runoff
                  wsoil_flx        , & ! water flux at interface
...
!-------------------------------------------------------------------------------
  ...
  INTEGER, INTENT(IN)  ::  &
                  ...
                  ivstart,           & ! start index for computations in the parallel program
                  ivend,             & ! end index for computations in the parallel program
                  iblock, itile,     & ! number of block, tile
                  ke_soil, ke_snow,  &
                  ke_soil_hy           ! number of active soil moisture layers
  ...
  REAL    (KIND = vpp), DIMENSION(nvec), INTENT(IN) :: &
                  ...
                  rsmin2d          , & ! minimum stomata resistance                    ( s/m )
! for TERRA-URB
                  isa              , & ! fraction of paved ared                          --
                  sa_uc            , & ! total impervious surface-area index
                  ai_uc            , & ! surface area index of the urban fabric
                  alb_red_uc       , & ! albedo reduction factor for the urban fabric
                  urb_fr_bld       , & ! Building area fraction for TERRA-URB          (  -  )
                                       !   with respect to urban tile
                  urb_h2w          , & ! Street canyon H/W ratio for TERRA-URB         ( m/m )
                  urb_h_bld        , & ! Building height for TERRA-URB                 (  m  )
...
! fields for new hydrology scheme
  REAL    (KIND = vpp), DIMENSION(nvec), OPTIONAL, INTENT(IN)  :: &
                  s_oro_max            ! mean grid point orography gradient             (  1  )

  REAL    (KIND = vpp), DIMENSION(nvec), OPTIONAL, INTENT(OUT) :: &
                  wt_depth         , & ! water table depth                              (  m  )
                  wso_satlev           ! first saturated level of watertable            (  -  )

  REAL    (KIND = vpp), DIMENSION(nvec,ke_soil+1), OPTIONAL, INTENT(OUT) :: &
                  wso_sat          , & ! soil moisture saturation                       (  -  )
                  q_roff           , & ! runoff
                  wsoil_flx            ! Water flux through layer interfaces

  REAL    (KIND = vpp), DIMENSION(nvec), OPTIONAL, INTENT(OUT) :: &
                  zshfl_sfc        , & ! sensible heat flux surface interface          (W/m2)
                  zlhfl_sfc        , & ! latent   heat flux surface interface          (W/m2)
!DR start
                  zqhfl_sfc            ! moisture heat flux surface interface          (W/m2)
!DR end

!--------------------------------------------------------------------------------

!#ifdef __COSMO__
!INTEGER :: tau_skin = 3600.0_vpp   ! NL variable in ICON for itype_canopy=2
!#endif
...
! -------------
  ...
  REAL    (KIND=vpp) ::  &
    ...
    zinf           , & ! infiltration
    ! for new hydrology scheme
    zred           , & ! reduction factor for impact of ice on aquifer discharge
    flux_ksom05    , & ! total flux at layer -1/2
    zklw_fr_ksom05_limit  , & ! hydraulic conductivity coefficient at half level below
                       ! (after flux correction)
    flux_ksom05_pos, & ! downward flux at layer +1/2
    flux_ksop05    , & ! total flux at layer +1/2
    flux_ksop05_neg, & ! upward flux at layer +1/2
    q_kso_pos      , & ! excess water to be transported one layer upward
    e_pos          , & ! positive evaporation from current layer
    alpha_lim      , & ! variable for flux limitation
    zgamma         , & ! scaling parameter for subsurf. runoff
  ...
  REAL(KIND=vpp)  ::  &
    ! for TERRA-URB
    zlambdaskin    , & ! skin conductivity                        ( W/m**2/K )
...
! -------------------------
  ...
  REAL(KIND = vpp)                 :: &
    ...
    zsoro          (nvec)              ! subgrid-scale slope of topography
  ...
  REAL    (KIND=vpp) ::  &
    ! for new hydrology scheme
    q_kso             (nvec,ke_soil+1), & ! Discharge from aquifer
    z_wt              (nvec)          , & ! depth of water table
    h_wt              (nvec)          , & ! distance of water table to next layer interface
    zdlw_fr_ksop05_fi (nvec,ke_soil+1), & ! hydraulic diffusivity coefficient at half level below
                                          ! the correction of the fluxes occurs only by modifying K.
                                          ! D is not adapted. Thus, there are no first- and second-guess
                                          ! values for D.
    zklw_fr_ksop05_fg (nvec,ke_soil+1), & ! hydraulic conductivity coefficient at half level below
                                          ! (first guess, before flux correction)
    zklw_fr_ksop05_sg (nvec,ke_soil+1), & ! hydraulic conductivity coefficient at half level below
                                          ! (second guess, corrected for oversaturation)

    ! for itype_hydmod==1 the final values of D and K need to be stored in an array, as a third loop
    ! for the computation of the matrix coefficients needs to be performed
    zklw_fr_ksop05_fi (nvec,ke_soil+1)    ! final hydraulic conductivity coefficient at half level below
  ...
  REAL    (KIND=vpp) ::  &
    ! utility variable for soil water budget
    zhwso_budget(nvec)                 ! all sources and sinks of soil moisture budget
...
#ifdef __SX__
  REAL(vpp) :: zfac(nvec),lhfl_pl_int(nvec)
#endif
...
! Just do some checkout prints:
  IF (ldebug) THEN
...
#endif
      WRITE(*,'(A,3I5)'   ) 'SFC-DIAGNOSIS terra start (ke_soil, ke_snow, ke_soil_hy):   ', ke_soil, ke_snow, ke_soil_hy
#ifdef _OPENMP
     ENDIF
#endif
    ENDIF
    ...
    IF (iblock == mbid .AND. my_cart_id == mcid) THEN
    !$acc wait
    !$acc update host(soiltyp_subs, plcov, rootdp, sai, tai, eai, rsmin2d, u, v, t, qv, ptot, ps, h_snow_gp, u_10m, v_10m)
    !$acc update host(prr_con, prs_con, conv_frac, prr_gsp, prs_gsp, prg_gsp, sobs, thbs, pabs, t_snow_now, t_s_now, t_g)
    !$acc update host(t_so_now, w_so_now, qv_s, w_snow_now, rho_snow_now, h_snow, w_i_now, w_p_now, w_s_now)
    !$acc update host(freshsnow, zf_snow, tch, tcm, tfv, runoff_s, runoff_g, skinc)
    IF (lterra_urb) THEN
    !$acc update host(isa, sa_uc, ai_uc, alb_red_uc, urb_fr_bld, urb_h2w)
    !$acc update host(urb_h2w, urb_h_bld, urb_alb_th, urb_alb_so, urb_hcap, urb_hcon)
    ENDIF
#ifdef TWOMOM_SB
    !$acc update host(prh_gsp)
#endif
    !$acc wait
    ENDIF
    ...
    DO i = ivstart, ivend
      IF (i== mvid .AND. iblock == mbid .AND. my_cart_id == mcid) THEN
       ...
       IF (my_thrd_id == mtid) THEN
#endif
        WRITE(*,'(A,3I5)'  ) 'SFC-DIAGNOSIS terra:  iblock = ', iblock, i, itile
        WRITE(*,'(A,I7,A14)') '  for time step and date = ', ntstep, yakdat1
      ...
      IF (lterra_urb) THEN
        WRITE(*,'(A,F28.16)') '   isa              :  ', isa         (i)
        WRITE(*,'(A,F28.16)') '   sa_uc            :  ', sa_uc       (i)
        WRITE(*,'(A,F28.16)') '   ai_uc            :  ', ai_uc       (i)
        WRITE(*,'(A,F28.16)') '   alb_red_uc       :  ', alb_red_uc  (i)
        WRITE(*,'(A,F28.16)') '   urb_fr_bld       :  ', urb_fr_bld  (i)
        WRITE(*,'(A,F28.16)') '   urb_h2w          :  ', urb_h2w     (i)
        WRITE(*,'(A,F28.16)') '   urb_h_bld        :  ', urb_h_bld   (i)
        WRITE(*,'(A,F28.16)') '   urb_alb_th       :  ', urb_alb_th  (i)
        WRITE(*,'(A,F28.16)') '   urb_alb_so       :  ', urb_alb_so  (i)
...
!------------------------------------------------------------------------------
  ...
  IF ( (itype_hydmod == 1) .AND. ( .NOT. PRESENT (s_oro_max)) ) THEN
    IF (my_cart_id == 0) THEN
      PRINT *, ' *** ERROR:  Argument s_oro_max   not present in TERRA for itype_hydmod=1! ***'
    ENDIF
    ierror = 2   ! s_oro_max needed for itype_hydmod = 1
    RETURN       ! whatever happens then
  ENDIF
...
! time constant for infiltration of water from interception store
  ...
  ! for urban parameterization
  !$acc present(isa, sa_uc, ai_uc, alb_red_uc, urb_fr_bld, urb_h2w)  &
  !$acc present(urb_h2w, urb_h_bld, urb_alb_th, urb_alb_so, urb_hcap)&
  !$acc present(urb_hcon, w_imp, w_isa)                              &
  ...
  ! Local arrays
  !$acc present(m_styp,ke_soil_hy_b,wso_sat)                         &
  ...
  !$acc present(zsatlev, zsoro, z_wt, h_wt, q_kso)                   &
  !$acc present(zdlw_fr_ksop05_fi, zklw_fr_ksop05_fg)                &
  !$acc present(zklw_fr_ksop05_sg, zklw_fr_ksop05_fi)                &
  ...
  ! for the optional fields for itype_hydmod=1
  !$acc data present(s_oro_max) if (itype_hydmod==1)
  !$acc data present(wso_satlev, wt_depth, q_roff, wsoil_flx) if (itype_hydmod==1)

  ! for the soil water budget
  !$acc data present(zhwso_budget, res_soilwatb) if (lres_soilwatb)
...
! scaling of ground water flow
  IF (itype_hydmod==1) THEN
    zgamma = 1.0_vpp/l_g
  ENDIF
...
! Prepare basic surface properties (for land-points only)
  ...
  DO i = ivstart, ivend
...
#ifndef __SX__
    zdw       (i,:) = cdw0  (mstyp) * soilhyd
    zdw1      (i,:) = cdw1  (mstyp)
    zkw       (i,:) = ckw0  (mstyp) * soilhyd
    zkwm      (i,:) = ckw0  (mstyp) * soilhyd
    zkw1      (i,:) = ckw1  (mstyp)
    ...
    zbwt      (i)   = MAX(0.001_vpp,rootdp(i)*fac_rootdp2)
                                                 ! Artificial minimum value for root depth
...
!NEC$ outerloop_unroll(8)
  DO kso = 1, ke_soil+1
    DO i = ivstart, ivend
    mstyp       = soiltyp_subs(i)        ! soil type
    zdw  (i,kso)  = cdw0 (mstyp) * soilhyd
    zdw1 (i,kso)  = cdw1 (mstyp)
    zkw  (i,kso)  = ckw0 (mstyp) * soilhyd
    zkwm (i,kso)  = ckw0 (mstyp) * soilhyd
...
!NEC$ outerloop_unroll(7)
  ...
  IF (itype_hydmod==1) THEN
    ...
    !$acc loop gang vector
    DO i = ivstart, ivend
        zsoro(i) = MAX (s_oro_max(i),s_oro_min)
    ENDDO
    !$acc end parallel
  ENDIF
...
!NEC$ outerloop_unroll(7)
  ...
  DO i = ivstart, ivend
    ...
    lhfl_bs(i)            = 0.0_vpp
    lhfl_pl(i,ke_soil+1)  = 0.0_vpp
    rstom  (i)            = 0.0_vpp
!   IF (lterra_urb .AND. (itype_eisa==2)) THEN
    zeisa(i)              = 0.0_vpp         ! only set for lterra_urb and itype_eisa==2
!   ENDIF
  ENDDO
  !$acc end parallel
...
!NEC$ outerloop_unroll(7)
  ...
  IF ( lterra_urb .AND. lurbfab) THEN
    ...
    ! - area index of buildings (ai_uc), height of building elements (urb_h_bld)
    !   and building fraction (sa_uc)
    ...
    DO kso = 1, ke_soil
      ...
      DO i = ivstart, ivend
        ...
        zalpha_uf  = MAX (0.0_vpp, MIN(zmls(kso)/urb_h_bld(i), 1.0_vpp))
        zalpha_lnd = MAX (0.0_vpp, MIN(zmls(kso)/c_lnd_h,      1.0_vpp))

        ! Changed by MV to be consistend with eq. (5) from Wouters et al., 2016
        zalam(i,kso)  = sa_uc(i) * (urb_hcon (i) * ai_uc(i) * (1.0_vpp - zalpha_uf) + &
                                    zalam(i,kso) * zalpha_uf) +                       &
                        (1.0_vpp - sa_uc(i)) * zalam(i,kso)
      ENDDO
    ENDDO
    !$acc end parallel
  END IF
...
!------------------------------------------------------------------------------
  ...
  DO i = ivstart, ivend
    ! ztsnow is now set according to itype_snow (see above);
...
!------------------------------------------------------------------------------
  ...
  DO i = ivstart, ivend
     ...
     IF (itype_snow /= 2) zrocs(i) = chc_i*zdz_snow_fl(i)*zrho_snow(i)
  ENDDO
  !$acc end parallel
      ...
      DO i = ivstart, ivend
        IF (zep_s(i) < 0.0_vpp) THEN   ! upwards directed potential evaporation
          ...
          zeisa(i) = zep_s(i) *(1.0_vpp - zf_snow(i)) & ! not snow covered
                    * isa(i)*c_isa_delt * (w_imp(i)/c_isa_wmax)**0.6667_vpp
...
!CDIR NODEP,VOVERTAKE,VOB
  ...
  DO i = ivstart, ivend
    ...
    zxx    = zf_snow(i)*zep_snow(i) + (1.0_vpp-zf_snow(i))*zep_s(i) ! snow-weighted potential evaporation
    IF (zxx < 0.0_vpp .AND. ze_sum < zxx) THEN
      zzz = zxx/ze_sum
...
#ifdef __SX__
                 (lhfl_pl_int(i)+MAX(0.2_vpp,plcov(i))*75.0_vpp) ))
#else
  ...
  IF (itype_interception == 1) THEN
    ...
    DO i = ivstart, ivend
      ...
      ze_sum = zdwsndt(i  )  & ! evaporation of snow
             ...
             + zeisa  (i  )    ! impervious surface evaporation: is 0 for not lterra_urb
...
!------------------------------------------------------------------------------
  ...
  ! diagnose water-table depth before the hydrological section starts
  IF (itype_hydmod==1) THEN
    !$acc parallel async
    !$acc loop gang vector
    DO   kso = 1,ke_soil+1
    ...
    !$acc parallel async
    !$acc loop gang vector
!US!!! NEC vs GPU: DO WHILE: what to do???
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3) THEN   ! neither ice nor rock as soil type
        zsatlev(i) = ke_soil_hy
        DO WHILE (((zw_fr(i,zsatlev(i))+100.0_vpp*eps_soil) >= zporv(i,INT(zsatlev(i)))) .AND. (zsatlev(i) > 0.0_vpp))
          zsatlev(i) = zsatlev(i)-1
        END DO
        zsatlev(i)    = zsatlev(i)+1
      END IF
    END DO
    ...
    !$acc parallel async
    !$acc loop gang vector
!US!!! this loop ordering is not good on NEC; and I think in this way also bad for GPU? kso should be outermost
    DO i = ivstart, ivend
      IF (soiltyp_subs(i)  >= 3) THEN
        IF (zsatlev(i)==1) THEN ! uppermost soil level
          h_wt(i) = MIN(zdzhs(1),MAX(0.0_vpp,zdzhs(1)*zw_fr(i,1)/zporv(i,1))) !dz in layer, liquid plus frozen
          z_wt(i) = zzhls(1) - h_wt(i) ! depth of water table
        ELSE
          DO   kso =2,ke_soil+1
            IF(kso==zsatlev(i)-1) THEN! last partly saturated level above ground water
              h_wt(i) = &
                MIN(zdzhs(kso),MAX(0.0_vpp,zdzhs(kso)*(zw_fr(i,kso)-zw_fr(i,kso-1))/(zporv(i,kso)-zw_fr(i,kso-1)+eps_div)))
                ! dz in layer, liquid plus frozen
              z_wt(i) = zzhls(kso) - h_wt(i) ! depth of water table
            END IF
          END DO
        END IF
      END IF
  ...
  IF (lterra_urb .AND. lurbfab) THEN
    ...
    DO   kso = 1, ke_soil+1
      ...
      DO i = ivstart, ivend
        ...
        ! Changed by MV to be consistend with eq. (5) from Wouters et al., 2016
        zroc(i,kso) = sa_uc(i) * (urb_hcap (i) * ai_uc(i) * (1.0_vpp - zalpha_uf) + &
                                  zroc(i,kso) * zalpha_uf) +                       &
                      (1.0_vpp - sa_uc(i)) * zroc(i,kso)
...
!------------------------------------------------------------------------------
  ...
  IF (itype_interception == 1) THEN
    !$acc parallel async
    !$acc loop gang worker vector private(zrime, zalf, zuv, zzz, zinf)     &
    !$acc private(zdwidtt, zdwieps, zro_wi)
    DO i = ivstart, ivend
      ! store forcing terms due to evapotranspiration, formation of dew
      ! and rime for later use
      zverbo(i) = zdwidt(i) + zesoil(i) + ztrangs(i) + zeisa(i) +     &
      ...
      IF ( (itype_snow == 2) .AND. (zwsnow(i) > 0.0_vpp) ) zalf = 1.0_vpp
    ...
    IF (lterra_urb .AND. (itype_eisa == 2)) THEN
      !$acc parallel async
      !$acc loop gang worker vector private(zisa_infil)
      DO i = ivstart, ivend
        ...
        w_isa(i)   = w_imp(i) * isa(i)

        zinfil(i) = (                                                    &
                     ! the water captured by non-impervious area
                     zvers(i) * (1.0_vpp - isa(i)) +                     &
                     ! the contribution captured by impervious area that gets infiltrated
                     zisa_infil * isa(i) * (1.0_vpp - c_isa_runoff)      & ! this is 0 because of c_isa_runoff=1
                    ) * zrock(i) * (1.0_vpp - zf_snow(i))                  ! consider rocks and snow fraction

        ! Add difference to surface runoff
        runoff_s(i) = runoff_s(i) + (zvers(i) - zinfil(i))*zroffdt
      END DO
      !$acc end parallel
    ...
    !$acc parallel async
    !$acc loop gang worker vector private(zdwsndtt, zwsnstr, zdwseps)
    DO i = ivstart, ivend
...
!------------------------------------------------------------------------------
! Section II.4a: Soil water transport and runoff from soil layers: original hydrology
!------------------------------------------------------------------------------

  IF (itype_hydmod==0) THEN
        ...
        DO i = ivstart, ivend
          ...
          IF (soiltyp_subs(i) >= 3) THEN
            ...
            IF (soiltyp_subs(i) == 8)  THEN
              ...
              zcou_roffg = MERGE(1.0_vpp, 0.0_vpp, kso<=ke_soil_hy_m)
            ELSE
              zfmb_fak   = MERGE(1.0_vpp, 0.0_vpp, kso==ke_soil_hy)
              zcou_roffg = MERGE(1.0_vpp, 0.0_vpp, kso<=ke_soil_hy)
            END IF
            ...
            runoff_g(i) = runoff_g(i) - (zrunoff_grav(i,kso) * zfmb_fak + zkorr) * zroffdt
...
!------------------------------------------------------------------------------
! Section II.4b: New (modified) version of soil-water transport and ground-water runoff
!------------------------------------------------------------------------------

  ELSEIF (itype_hydmod==1) THEN

    ! Published in Schlemmer et al. 2018, Ported to NWP version by Daniel Regenass
    ! and Linda Schlemmer (IACETH)
    ...
    !$acc parallel async
    !$acc loop gang vector private (zice_fr_kso, zice_fr_ksop1, zlw_fr_kso, zlw_fr_ksop1) &
    !$acc private (zfr_ice, zredp05, zlw_fr_ksop05)
    DO i = ivstart, ivend
      IF (soiltyp_subs(i)  >= 3) THEN
        ! uppermost layer, kso = 1
        ! sedimentation and capillary transport in soil
        ...
        zdlw_fr_ksop05_fi(i,1) = zredp05 * watrdiff_RT(zdw(i,1),zlw_fr_ksop05, zdw1(i,1),zporv(i,1),zadp(i,1))

        IF (zsatlev(i)==2) THEN ! last level above saturation, partially saturated
          zklw_fr_ksop05_fg(i,1) = zredp05 * watrcon_RT (zkw(i,1),zlw_fr_ksop05, zkw1(i,1),zporv(i,1),zadp(i,1))
          q_kso(i,1) = zredp05*zsoro(i)*zgamma*rho_w*zkw(i,1)*h_wt(i)/zdzhs(1) ! saturated ground water flow at the bottom
        ELSE iF (2 >= zsatlev(i)) THEN ! entirely saturated
          zklw_fr_ksop05_fg(i,1) = 0.0_vpp
          q_kso(i,1) = zredp05*zsoro(i)*zgamma*rho_w*zkw(i,1)
        ELSE ! default case, unsaturated
!US??     zklw_fr_ksop05_fg(i,1) = zredp05*watrcon_RT(zkw(i,1),zlw_fr_ksop05,zkw1(i,1),zporv(i,1),zadp(i,kso))
!US??     q_kso(i,kso) = 0.0_vpp
          zklw_fr_ksop05_fg(i,1) = zredp05*watrcon_RT(zkw(i,1),zlw_fr_ksop05,zkw1(i,1),zporv(i,1),zadp(i,1))
          q_kso(i,1) = 0.0_vpp
        END IF
    ...
    !$acc loop seq
     DO   kso =2,ke_soil_hy-1
      !$acc loop gang vector                                                    &
      !$acc private(zice_fr_ksom1, zice_fr_kso, zice_fr_ksop1              )    &
      !$acc private(zlw_fr_ksom1,  zlw_fr_kso,  zlw_fr_ksop1               )    &
      !$acc private(zlw_fr_ksom05, zlw_fr_ksop05, zfr_ice, zredm05, zredp05)
      DO i = ivstart, ivend
        IF (soiltyp_subs(i) >= 3) THEN   ! neither ice nor rock as soil type
          ! sedimentation and capillary transport in soil
          ...
          ! first guess K
          IF ((kso+1)==zsatlev(i)) THEN ! last level above saturation, partially saturated

            zdlw_fr_ksop05_fi(i,kso)= zredp05*watrdiff_RT(zdw(i,kso),zlw_fr_ksop05,&
                                          zdw1(i,kso),zporv(i,kso),zadp(i,kso))
            zklw_fr_ksop05_fg(i,kso)= zredp05*watrcon_RT(zkw(i,kso),zlw_fr_ksop05,zkw1(i,kso),zporv(i,kso),zadp(i,kso))
            q_kso(i,kso) = zredp05*zsoro(i)*zgamma*rho_w*zkw(i,kso)*h_wt(i)/zdzhs(kso) ! saturated ground water flow at the bottom

          ELSE iF (kso >= zsatlev(i)) THEN ! entirely saturated

            zklw_fr_ksop05_fg(i,kso) = 0.0_vpp

            q_kso(i,kso) = zredp05*zsoro(i)*zgamma*rho_w*zkw(i,kso)
    ...
    !$acc parallel async
    !$acc loop gang vector                                                    &
    !$acc private(zice_fr_ksom1, zice_fr_kso                             )    &
    !$acc private(zlw_fr_ksom1,  zlw_fr_kso, zlw_fr_ksom05, zfr_ice, zred)
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3) THEN
       ! lowest active hydrological layer ke_soil_hy-1
        ...
        zfr_ice          = MAX (zice_fr_kso,zice_fr_ksom1)
        zred = 1.0_vpp-zfr_ice/  &
               MAX (zlw_fr_kso+zice_fr_kso,zlw_fr_ksom1+zice_fr_ksom1)

            ! These two quantities are never used as the zero flux bottom boundary
            ! condition directly enters the tridiagonal systems as a 0 for zdlw_fr_ksop05(i,ke_soil_hy)
            ! as this would be the only reason to create an array instead of a scalar (which
            ! produces a conflict with itype_hydmod = 0, we ommit it. 
            ! DR        zdlw_fr_ksop05(i,ke_soil_hy) = 0.0_vpp
            ! DR
            ! DR        zklw_fr_ksop05(i,ke_soil_hy) = 0.0_vpp

        IF (ke_soil_hy >= zsatlev(i)) THEN ! fully saturated

          q_kso(i,ke_soil_hy) = zred*zsoro(i)*zgamma*rho_w*zkw(i,ke_soil_hy)

        ELSE ! partially saturated

           q_kso(i,ke_soil_hy) = zred*zsoro(i)*zgamma*rho_w*zkw(i,ke_soil_hy)*h_wt(i)/zdzhs(ke_soil_hy)
    ...
    ! Check for overdepletion: limit the fluxes both going into the layer and
    ! going out of the layer
    ! Check for oversaturation: limit the fluxes going into the layer, at km05

    !$acc parallel async
    !$acc loop gang vector                                                    &
    !$acc private(zice_fr_ksom1, zice_fr_kso, zlw_fr_ksom1, zlw_fr_kso   )    &
    !$acc private(zlw_fr_ksom05, flux_ksom05, flux_ksom05_pos, q_kso_pos )    &
    !$acc private(e_pos, alpha_lim, zklw_fr_ksom05_limit                 )
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3) THEN
         ! lowest active hydrological layer ke_soil_hy-1
         ...
         ! limit the first guess fluxes such that no depletion can occur

         flux_ksom05     = (zdlw_fr_ksop05_fi(i,ke_soil_hy-1)*(zlw_fr_kso+zice_fr_kso-zlw_fr_ksom1-zice_fr_ksom1)/ &
                            zdzms(ke_soil_hy) - zklw_fr_ksop05_fg(i,ke_soil_hy-1))/zdzhs(ke_soil_hy)
         flux_ksom05_pos = max(0.0_vpp,flux_ksom05)
         q_kso_pos       = max(0.0_vpp,q_kso(i,ke_soil_hy)/rho_w)
         e_pos           = max(0.0_vpp,ztrang(i,ke_soil_hy)/rho_w/zdzhs(kso))
         alpha_lim       = max(0.0_vpp,MIN(1.0_vpp,&
                                 zlw_fr_kso/(zdt*(flux_ksom05_pos+q_kso_pos+e_pos+eps_soil))))

         zklw_fr_ksop05_sg(i,ke_soil_hy-1) = alpha_lim *zklw_fr_ksop05_fg(i,ke_soil_hy-1)
         q_kso(i,ke_soil_hy)               = alpha_lim * q_kso(i,ke_soil_hy)

         !if(alpha_lim.lt.1.0_vpp) then
         !   print*,'limit alpha',ke_soil_hy,
         !   zlw_fr(i,ke_soil_hy),zklw_fr_ksop05_fg(i,ke_soil_hy-1),zklw_fr_ksop05_sg(i,ke_soil_hy-1),
         !   alpha_lim, zdlw_fr_ksop05_fi(i,ke_soil_hy-1),q_kso(i,ke_soil_hy)
         !end if

         ! limit second guess fluxes and K such that no oversaturation can occur in this
         ! layer
         zklw_fr_ksom05_limit = (zporv(i,ke_soil_hy)-zlw_fr_kso-zice_fr_kso)/zdt*zdzhs(ke_soil_hy) &
         - (q_kso(i,ke_soil_hy)*zdzhs(ke_soil_hy)+ztrang(i,ke_soil_hy))/rho_w &
         + zdlw_fr_ksop05_fi(i,ke_soil_hy-1) &
         *(zlw_fr_kso+zice_fr_kso-zlw_fr_ksom1-zice_fr_ksom1)/zdzms(ke_soil_hy)

         zklw_fr_ksop05_fi(i,ke_soil_hy-1) = MIN(zklw_fr_ksop05_sg(i,ke_soil_hy-1),zklw_fr_ksom05_limit)
    ...
    DO kso = ke_soil_hy-1,2,-1
      !$acc loop gang vector                                                    &
      !$acc private(zice_fr_ksom1, zice_fr_kso, zice_fr_ksop1              )    &
      !$acc private(zlw_fr_ksom1, zlw_fr_kso, zlw_fr_ksop1, flux_ksom05    )    &
      !$acc private(flux_ksop05, flux_ksom05_pos, flux_ksop05_neg          )    &
      !$acc private(q_kso_pos, e_pos, alpha_lim, zklw_fr_ksom05_limit      )
      DO i = ivstart, ivend
        IF (soiltyp_subs(i) >= 3) THEN
          ! sedimentation and capillary transport in soil
          zice_fr_ksom1 = ziw_fr(i,kso-1)
          ...
          ! limit first guess K such that no overdepletion of this
          ! layer can occur

          flux_ksom05 = (zdlw_fr_ksop05_fi(i,kso-1)* &
                        (zlw_fr_kso+zice_fr_kso-zlw_fr_ksom1-zice_fr_ksom1)/ &
                         zdzms(kso) - zklw_fr_ksop05_fg(i,kso-1))/zdzhs(kso)
          flux_ksop05 = (zdlw_fr_ksop05_fi(i,kso)* &
                        (zlw_fr_ksop1+zice_fr_ksop1-zlw_fr_kso-zice_fr_kso)/ &
                         zdzms(kso+1) - zklw_fr_ksop05_fi(i,kso))/zdzhs(kso)
          flux_ksom05_pos = max(0.0_vpp,flux_ksom05)
          flux_ksop05_neg = min(0.0_vpp,flux_ksop05)
          q_kso_pos       = max(0.0_vpp,q_kso(i,kso)/rho_w)
          e_pos           = max(0.0_vpp,ztrang(i,kso)/rho_w/zdzhs(kso))
          alpha_lim       = max(0.0_vpp,min(1.0_vpp,&
                                 zlw_fr_kso/(zdt*(-flux_ksop05_neg+flux_ksom05_pos+q_kso_pos+e_pos+eps_soil))))

          zklw_fr_ksop05_sg(i,kso-1) = alpha_lim * zklw_fr_ksop05_fg(i,kso-1)
          zklw_fr_ksop05_fi(i,kso)   = alpha_lim * zklw_fr_ksop05_fi(i,kso)
          q_kso(i,kso)               = alpha_lim * q_kso(i,kso)
          !if(alpha_lim.lt.1.0_vpp) then
          !  print*,'limit alpha',kso,
          !  zklw_fr_ksop05_sg(i,kso-1),zklw_fr_ksop05(i,kso-1), alpha_lim
          !end if

          ! limit these second guess fluxes such that no oversaturation can occur

          zklw_fr_ksom05_limit = (zporv(i,kso)-zlw_fr_kso-zice_fr_kso)/zdt*zdzhs(kso) &
                               - (q_kso(i,kso)*zdzhs(kso)+ztrang(i,kso))/rho_w &
                               + zdlw_fr_ksop05_fi(i,kso-1)*(zlw_fr_kso  +zice_fr_kso  -zlw_fr_ksom1-zice_fr_ksom1)/zdzms(kso)&
                               - zdlw_fr_ksop05_fi(i,kso)  *(zlw_fr_ksop1+zice_fr_ksop1-zlw_fr_kso  -zice_fr_kso)  /zdzms(kso+1) &
                               + zklw_fr_ksop05_fi(i,kso)
          zklw_fr_ksop05_fi(i,kso-1) = min(zklw_fr_ksop05_sg(i,kso-1),zklw_fr_ksom05_limit)

          !soil water flux, explicit part, for soil water flux investigations only)
          zflmg(i,kso) = rho_w &
                       *(zdlw_fr_ksop05_fi(i,kso-1)*(zlw_fr_kso+zice_fr_kso-zlw_fr_ksom1-zice_fr_ksom1) &
                       / zdzms(kso) - zklw_fr_ksop05_fi(i,kso-1))
        ENDIF
      END DO
    END DO
    !$acc end parallel
    ...
    !$acc parallel async
    !$acc loop gang vector                                                    &
    !$acc private(zice_fr_kso, zice_fr_ksop1, zlw_fr_kso, zlw_fr_ksop1   )    &
    !$acc private(flux_ksom05, flux_ksop05, flux_ksop05_neg              )    &
    !$acc private(flux_ksom05_pos, q_kso_pos, e_pos, alpha_lim           )    &
    !$acc private(zklw_fr_ksom05_limit, z1dgam1, zgam2p05                )
    DO i = ivstart, ivend
      IF (soiltyp_subs(i)  >= 3) THEN

        ! limit first-guess fluxes such that no depletion can occur
        ! the flux from above at km05 is the infiltration, which is further limited at
        ! this point and excess is added to the surface runoff.
        ...
        ! Overdepletion
        flux_ksom05 = -zinfil(i)/rho_w
        flux_ksop05 = (zdlw_fr_ksop05_fi(i,1)*(zlw_fr_ksop1+zice_fr_ksop1-zlw_fr_kso-zice_fr_kso)/  &
                       zdzms(2) - zklw_fr_ksop05_fi(i,1))/zdzhs(1)
        flux_ksop05_neg = MIN(0.0_vpp,flux_ksop05)
        flux_ksom05_pos = MAX(0.0_vpp,flux_ksom05)
        q_kso_pos       = MAX(0.0_vpp,q_kso(i,1)/rho_w)
        e_pos           = MAX(0.0_vpp,(ztrang(i,1)+zesoil(i))/rho_w/zdzhs(1))
        alpha_lim       = MAX(0.0_vpp,MIN(1.0_vpp,&
                                 zlw_fr(i,1)/(zdt*(-flux_ksop05_neg+flux_ksom05_pos+q_kso_pos+e_pos+eps_soil))))

        zklw_fr_ksop05_fi(i,1) = alpha_lim * zklw_fr_ksop05_fi(i,1)
        q_kso(i,1)             = alpha_lim * q_kso(i,1)
        !if(alpha_lim.lt.1.0_vpp) then
        !   print*,'limit alpha uppermost layer', zklw_fr_ksop05(i,1),
        !   alpha_lim
        !end if

        ! limit second guess K such that no oversaturation can occur in this layer
        zklw_fr_ksom05_limit = (zporv(i,1)-zlw_fr(i,1)-zice_fr_kso)/zdt*zdzhs(1) &
                               + q_kso(i,1)*zdzhs(1)/rho_w + ztrang(i,1)/rho_w + zesoil(i)/rho_w &
                               - zdlw_fr_ksop05_fi(i,1)*(zlw_fr_ksop1+zice_fr_ksop1-zlw_fr_kso-zice_fr_kso)/zdzms(2) &
                               + zklw_fr_ksop05_fi(i,1)
      
         runoff_s(i) = runoff_s(i) + max(0.0_vpp, zinfil(i) - zklw_fr_ksom05_limit*rho_w)*zdt
         zinfil(i) = min(zinfil(i),zklw_fr_ksom05_limit*rho_w)

        ! coefficients for implicit flux computation
        z1dgam1     = zdt/zdzhs(1)
        zgam2p05    = zdlw_fr_ksop05_fi(i,1)/zdzms(2)
        zaga(i,1) = 0.0_vpp
        zagb(i,1) = 1.0_vpp+zalfa*zgam2p05*z1dgam1
        zagc(i,1) = -zalfa * zgam2p05*z1dgam1
        zagd(i,1) = zlw_fr(i,1) + zinfil(i)*z1dgam1/rho_w  &
                     -zklw_fr_ksop05_fi(i,1)*z1dgam1                     &
                     +(1.0_vpp - zalfa)* zgam2p05*z1dgam1*(zlw_fr_ksop1  - zlw_fr_kso)  &
                     +                   zgam2p05*z1dgam1*(zice_fr_ksop1 - zice_fr_kso)
    ...
    ! in a third loop from top to bottom the coefficients for the matrix with 
    ! the final, limited fluxes are determined.
    ...
    DO kso = ke_soil_hy-1,2,-1
      ...
      !$acc private(zice_fr_ksom1, zice_fr_kso, zice_fr_ksop1              )    &
      !$acc private(zlw_fr_ksom1, zlw_fr_kso, zlw_fr_ksop1                 )    &
      !$acc private(z1dgam1, zgam2m05, zgam2p05                            )
      DO i = ivstart, ivend
        IF (soiltyp_subs(i) >= 3) THEN
          ! sedimentation and capillary transport in soil
          zice_fr_ksom1 = ziw_fr(i,kso-1)
          ...
          ! coefficients for implicit flux computation
          z1dgam1 = zdt/zdzhs(kso)
          zgam2m05  = zdlw_fr_ksop05_fi(i,kso-1)/zdzms(kso)
          zgam2p05  = zdlw_fr_ksop05_fi(i,kso)/zdzms(kso+1)

          zaga (i,kso) = -zalfa*zgam2m05*z1dgam1
          zagc (i,kso) = -zalfa*zgam2p05*z1dgam1
          zagb (i,kso) = 1.0_vpp +zalfa*(zgam2m05+zgam2p05)*z1dgam1
          zagd (i,kso) = zlw_fr(i,kso)+                              &
                         z1dgam1*(-zklw_fr_ksop05_fi(i,kso) + zklw_fr_ksop05_fi(i,kso-1))+ &
                         (1.0_vpp-zalfa)*z1dgam1*                     &
                         (zgam2p05*(zlw_fr_ksop1-zlw_fr_kso  )       &
                         -zgam2m05*(zlw_fr_kso  -zlw_fr_ksom1)   )   &
                         +z1dgam1*                                   &
    ...
    !$acc loop gang vector                                                    &
    !$acc private(zice_fr_ksom1, zice_fr_kso, zlw_fr_ksom1, zlw_fr_kso   )    &
    !$acc private(zlw_fr_ksom05, z1dgam1, zgam2m05                       )
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3) THEN
         ! lowest active hydrological layer ke_soil_hy-1
         ...
         z1dgam1 = zdt/zdzhs(ke_soil_hy)
         zgam2m05  = zdlw_fr_ksop05_fi(i,ke_soil_hy-1)/zdzms(ke_soil_hy)

         zaga(i,ke_soil_hy) = -zalfa* zgam2m05*z1dgam1
         zagb(i,ke_soil_hy) = 1.0_vpp+ zalfa*zgam2m05*z1dgam1
         zagc(i,ke_soil_hy) = 0.0_vpp
         zagd(i,ke_soil_hy) =zlw_fr(i,ke_soil_hy)+z1dgam1*zklw_fr_ksop05_fi(i,ke_soil_hy-1) &
                           +(1.0_vpp-zalfa)*z1dgam1* &
                            zgam2m05*(zlw_fr_ksom1  - zlw_fr_kso)            &
                           +z1dgam1*                                         &
                            zgam2m05*(zice_fr_ksom1 - zice_fr_kso)
...
!CDIR NODEP,VOVERTAKE,VOB
!DIR$ IVDEP
      !$acc loop gang vector private(zdwg, zwgn, zro2, zkorr, zro)
      DO i = ivstart, ivend
        IF (soiltyp_subs(i) >= 3) THEN
          ...
          !zdwg   =  zflmg(i,kso+1) - zflmg(i,kso)
          !zdwg calculated above by flux divergence has to be aequivalent with 
          ! convert solution from implicit solution to explicit time
          ! derivative
          ! and add evaporation and transpiration and remove runoff
          ...
          ! and transpiration (for each layer)
          zdwg   = zdwg + znlgw1f(kso) * zesoil(i) + ztrang (i,kso)
          zwgn   = zw_fr(i,kso) + zdtdrhw*zdwg/zdzhs(kso) - zdtdrhw*q_kso(i,kso)
          ! correct if water content exceeds porosity or is below adp
          zro2   = zrhwddt*zdzhs(kso)*MAX(0.0_vpp, zwgn - zporv(i,kso))
          zkorr  = zrhwddt*zdzhs(kso)*MAX(0.0_vpp, zadp(i,kso) - zwgn )
          zdwgdt(i,kso)= zdwg + zkorr - zro2
          zro    = zro2
          ...
          wsoil_flx(i,kso) = zflmg(i,kso)
        END IF          ! ice/rock-exclusion
      END DO
    END DO         ! end loop over soil layers
    !$acc end parallel

    ! second contribution to ground runoff: from aquifer
    !$acc parallel async
    DO  kso = 1,ke_soil_hy
      !$acc loop gang vector
      DO i = ivstart, ivend
        IF (soiltyp_subs(i) >= 3) THEN
          ! remove runoff
          zdwgdt(i,kso) = zdwgdt(i,kso) - q_kso(i,kso)*zdzhs(kso)
          runoff_g(i)   = runoff_g(i)+ q_kso(i,kso)*zdzhs(kso)*zdt
        END IF
      END DO
    END DO
    !$acc end parallel

    IF (PRESENT (wt_depth) .AND. PRESENT(wso_satlev) .AND. PRESENT(q_roff) .AND. PRESENT(wsoil_flx)) THEN
      ! save diagnostic variables for output
      !$acc parallel async
      !$acc loop gang vector
      DO i = ivstart, ivend
        IF (soiltyp_subs(i) >= 3) THEN
          wt_depth  (i) = z_wt(i)
          wso_satlev(i) = REAL(zsatlev(i),vpp)
        ELSE ! otherwise there are problems with undefined points in GRIB output
          wt_depth  (i) = 0.0_vpp
          wso_satlev(i) = 0.0_vpp
        ENDIF
      ENDDO
      !$acc end parallel

      !$acc parallel async
      DO  kso = 1,ke_soil_hy  ! not ke_soil+1
        !$acc loop gang vector
        DO i = ivstart, ivend
          IF (soiltyp_subs(i) >= 3) THEN
            q_roff   (i,kso) = q_kso(i,kso)
            wsoil_flx(i,kso) = zflmg(i,kso)
          ELSE ! otherwise there are problems with undefined points in GRIB output
            q_roff   (i,kso) = 0.0_vpp
            wsoil_flx(i,kso) = 0.0_vpp
          ENDIF
        ENDDO
      ENDDO
      !$acc end parallel
  ...
  IF (itype_snow == 2) THEN
    ...
    DO i = ivstart, ivend
      ...
      IF (lterra_urb .AND. lurbfab) THEN
        ...
        ztalb = sa_uc(i) * urb_alb_th(i) * alb_red_uc(i) + (1.0_vpp - sa_uc(i)) * Ctalb
      ELSE
        ztalb = Ctalb
      END IF
...
!NEC$ sparse
    DO i = ivstart, ivend
      ...
      ! thawing of snow falling on soil with Ts > T0
      !
      ! it turned out that ensuring (ztsnow_pm(i)*zrs(i) > 0.0_vpp) is not sufficient, as in rare cases
...
!------------------------------------------------------------------------------
  ...
  ! (see below, after the statement IF (itype_snow==2) THEN);
  ...
  DO kso = 2, ke_soil
    DO i = ivstart, ivend
      IF ((itype_snow /= 2) .OR. (sn_frac(i) < 1.0_vpp) ) THEN
  ...
  DO i = ivstart, ivend
!    IF ((itype_snow /= 2) .OR. (zf_snow(i) < 1.0_vpp .AND. zwsnew(i).GT.eps_soil)) THEN
    IF ((itype_snow /= 2) .OR. (sn_frac(i) < 1.0_vpp) ) THEN
...
! melting snow. In cases this melting may be postponed to the next time step.
  ...
  ELSEIF (itype_snow == 2) THEN       ! multi-layer snow scheme from Ekaterina
...
!------------------------------------------------------------------------------
  ...
  DO i = ivstart, ivend
    ...
    ELSE IF (itype_canopy == 2) THEN
      ...
      ELSE
        IF (lterra_urb .AND. lurbfab) THEN
          zlambdaskin = cskinc_urb *  sa_uc(i)  + skinc(i)   * (1.0_vpp - sa_uc(i))
        ELSE
          zlambdaskin = MAX(5.0_vpp,skinc(i))
        ENDIF
        t_sk_new(i) = t_sk_now(i) + 0.5_vpp*(t_s_new(i) - t_s_now(i)) + zdt/tau_skin *                           &
          ( (zrnet_s(i) + zshfl_s(i) + zlhfl_s(i) + zsprs(i))/zlambdaskin - (t_sk_now(i) - t_s_now(i)) )
      ENDIF
...
! GZ: this additional computation is obsolete with snow tiles because grid points
!     with completely melted snow are deleted afterwards
#ifndef __ICON__
...
! end of ifndef __ICON__: obsolete additional computation
#endif
...
!NEC$ sparse
 ...
 ENDIF ! itype_snow
  ...
  ! Diagnostic output of Saturation
  IF ( (itype_hydmod == 1) .AND. PRESENT (wso_sat) ) THEN
    ! After Schlemmer et al. (2018) dynamic range goes from air dry point to pore volume
    !$acc parallel async
    !$acc loop gang vector collapse(2)
    DO kso = 1, ke_soil+1
      DO i = ivstart, ivend
        wso_sat(i,kso) = sat_pv(w_so_new(i,kso), zporv(i,kso), zadp(i,kso), zdzhs(kso))
      ENDDO
    ENDDO
    !$acc end parallel
  ELSE
    ! Dynamic range from air dry point to field capacity
    !$acc parallel async
    !$acc loop gang vector collapse(2)
    DO kso = 1, ke_soil+1
      DO i = ivstart, ivend
        wso_sat(i,kso) = sat_fcap(w_so_new(i,kso), zfcap(i,kso), zadp(i,kso), zdzhs(kso))
      ENDDO
    ENDDO
    !$acc end parallel
  ENDIF

  IF (itype_snow /= 3) THEN
  ...
  IF (lres_soilwatb) THEN
    ...
    DO i = ivstart, ivend
      ...
      zhwso_budget(i) = zhwso_budget(i) - ( w_snow_new(i) - w_snow_now(i) )*1000.0_vpp
      zhwso_budget(i) = zhwso_budget(i) - ( w_i_new(i)    - w_i_now(i)    )*1000.0_vpp
      IF (itype_interception == 2) THEN
        zhwso_budget(i) = zhwso_budget(i) - ( w_p_new(i)    - w_p_now(i)    )*1000.0_vpp
        zhwso_budget(i) = zhwso_budget(i) - ( w_s_new(i)    - w_s_now(i)    )*1000.0_vpp
      ENDIF
      ! residuum = recharge - budget (RHS)
      res_soilwatb(i) = res_soilwatb(i) + ( ( SUM(w_so_new(i,1:ke_soil_hy)) - & 
        SUM(w_so_now(i,1:ke_soil_hy)) )*1000.0_vpp - zhwso_budget(i) )
    END DO
    !$acc end parallel
  ENDIF
...
!!!!#ifdef __ICON__
  IF (ldebug) THEN
    IF (iblock == mbid .AND. my_cart_id == mcid) THEN
    !$acc wait
    !$acc update host(t_s_new, t_snow_new, w_snow_new, rho_snow_new, h_snow_new, sn_frac, w_i_new, w_s_new, w_p_new)
    !$acc update host(t_so_new, w_so_new, w_so_ice_new, runoff_s, runoff_g, rstom, zshfl_sfc, zlhfl_sfc)
    !$acc update host(zqhfl_sfc, zshfl_s, zlhfl_s, zshfl_snow, zlhfl_snow, zgsb, lhfl_bs, lhfl_pl)
    IF (itype_hydmod==1) THEN
    !$acc update host(wt_depth, wsoil_flx, q_roff, wso_satlev, wso_sat)
    ENDIF
#ifdef __ICON__
    IF (itype_trvg == 3) THEN
      !$acc update host(plevap)
    ENDIF
#endif
    !$acc wait
    ENDIF
...
!     IF (ABS(t_s_now(i)-t_s_new(i)) > 25.0_vpp) THEN
      IF (i == mvid .AND. iblock == mbid .AND. my_cart_id == mcid) THEN
       ...
       IF (my_thrd_id == mtid) THEN
        ...
        WRITE(*,'(A, F28.16)') '   fresh_snow (out) :  ', freshsnow(i)
        ...
        IF (lterra_urb .AND. itype_eisa==2) THEN
        WRITE(*,'(A,2F28.16)') '   w_imp            :  ', w_imp(i)
        WRITE(*,'(A,2F28.16)') '   w_isa            :  ', w_isa(i)
        ENDIF
        DO k = 0, ke_soil+1
        ...
        ENDDO
        WRITE(*,'(A        )') '                                '
        WRITE(*,'(A        )') ' Fluxes and soil parameters calculated by TERRA:  '
        ...
        IF (itype_hydmod == 1) THEN
          WRITE(*,'(A        )') '                                '
          WRITE(*,'(A        )') ' itype_hydmod == 1:  '
          WRITE(*,'(A, F28.16)') '   wt_depth         :  ', wt_depth(i)
          WRITE(*,'(A, F28.16)') '   wso_satlev       :  ', wso_satlev(i)
          DO k = 1, ke_soil_hy
            WRITE(*,'(A,I1,A, F28.16)') '   q_roff   (',k,')     :  ', q_roff   (i,k)
          ENDDO
          DO k = 1, ke_soil_hy
            WRITE(*,'(A,I1,A, F28.16)') '   wsoil_flx(',k,')     :  ', wsoil_flx(i,k)
          ENDDO
          DO k = 1, ke_soil+1
            WRITE(*,'(A,I1,A, F28.16)') '   wso_sat  (',k,')     :  ', wso_sat  (i,k)
          ENDDO
        ENDIF
...
! for optional field s_oro_max
!$acc end data 

! for optional fields wt_depth, wso_satlev, wsoil_flx, q_roff
!$acc end data 

! for optional fields plevap, z0
!$acc end data 

! for field zhwso_budget
!$acc end data 
...
! zsf_dqvdt_iw = z234iw*(1.0_vpp + REAL(rvd_m_o_wp,vpp)*zsqsatx)*zsqsatx/(zstx - z4iw)**2
  zsf_dqvdt_iw = z234iw*(1.0_vpp +      rvd_m_o        *zsqsatx)*zsqsatx/(zstx - z4iw)**2
...
REAL (KIND=vpp) FUNCTION sat_pv(wso, pv, adp, dz)
  !Saturation with respect to pore volume
  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  ::  wso, pv, adp, dz
  sat_pv = (wso/dz - adp) / (pv -adp)

END FUNCTION sat_pv

!==============================================================================

REAL (KIND=vpp) FUNCTION sat_fcap(wso, fc, adp, dz)
  !Saturation with respect to field capacity
  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  ::  wso, fc, adp, dz
  sat_fcap = (wso/dz - adp) / (fc - adp)

END FUNCTION sat_fcap
